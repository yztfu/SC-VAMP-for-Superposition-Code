...
def oamp_pa_linear(config:OAMPConfiguration, r1:np.ndarray, gamma1:float):
    x1, alpha1 = config.lmmse_esti_pa(r1=r1, y=config.y_pa, gamma1=gamma1, snr=config.snr, A_mat=config.A_pa)
    eta1 = gamma1 / alpha1
    gamma2 = eta1 - gamma1
    r2 = (eta1 * x1 - gamma1 * r1) / gamma2

    x_post, v_post = config.denoiser_bayes_pa(beta_pri=r2, var_pri=(1/gamma2), rho=config.rho, N_all=config.N_all, B_src=config.B_src, eps=config.eps, Pl=config.Pl)
...
def se_oamp_pa_linear(config:OAMPConfiguration, v_A_pri:float):
    # TODO support the power allocation
    alpha1 = config.lmmse_esti_pa(gamma1=gamma1, snr=config.snr, A_mat=config.A_ori, se=True)
    eta1 = gamma1 / alpha1
    gamma2 = eta1 - gamma1

    # online average of x
    r2x = config.x_prior_generate_pa(N_all=config.N_se, rho=config.rho, B_src=config.B_src, sigma2=config.sigma2)
    r2z = np.random.normal(0, np.sqrt(1/gamma2), (config.N_se, 1))
    r2 = r2x + r2z
    x_post, v_post = config.denoiser_bayes_pa(beta_pri=r2, var_pri=(1/gamma2), rho=config.rho, N_all=config.N_se, B_src=config.B_src, eps=config.eps, Pl=config.Pl)
...
def oamp_pa_initialization(config:OAMPConfiguration):
    r1 = np.zeros((config.N_all, 1))
    gamma1 = 1
    return r1, gamma1


def se_oamp_pa_initialization(config:OAMPConfiguration):
    gamma1 = 1
    return gamma1
...
def output_from_error_algo(dt, ser_gt, mse_gt, mse_esti):
    logging.info(f"*** Iteration: {dt:03d} ***")
    logging.info(f"   *MEAN* | ser_gt: {ser_gt:.4e} | mse_gt: {mse_gt:.4e} | "
                 f"mse_esti: {mse_esti:.4e}")


def iteration_oamp_pa(config:OAMPConfiguration):
    '''
    The main iteration of oamp_pa
    '''
    # Print configuration
    # config.config_print()
    logging.info(f"oamp_ori iterations start...")
    # Initialization
    r1, gamma1 = oamp_pa_initialization(config=config)
    # gamma1_se = se_oamp_pa_initialization(config=config)
    ...
    for dt in range(config.T):
        r1, gamma1, x_post, v_post = oamp_pa_linear(config, r1, gamma1)
        ser_gt, mse_gt = ga_calculation(x_post, config.x_all_pa, config.B_src)
        mse_esti = v_post
        ser_gt_ite[:, dt] = ser_gt
        mse_gt_ite[:, dt] = mse_gt
        mse_esti_ite[:, dt] = mse_esti

        # gamma1_se, v_post_se, x_all_se, x_post_se = se_oamp_pa_linear(config, gamma1_se)
